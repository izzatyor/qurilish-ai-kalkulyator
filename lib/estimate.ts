...
const TIERS: Record<
  Tier,
  { label: string; priceFactor: number; laborPerM2: number; floor: string; floorPrice: number }
> = {
  ekonom: { label: 'Ekonom', priceFactor: 0.8, laborPerM2: 160_000, floor: 'Linoleum 2.5 mm', floorPrice: 62_000 },
  standart: { label: 'Standart', priceFactor: 1, laborPerM2: 230_000, floor: 'Laminat 32-klass', floorPrice: 98_000 },
  premium: { label: 'Premium', priceFactor: 1.55, laborPerM2: 360_000, floor: 'Parket taxta', floorPrice: 240_000 },
}
...
export function estimate(input: RoomInput): Estimate {
  ...
  const add = (name: string, spec: string, qty: number, unit: string, unitPrice: number) => {
    const price = roundTo(unitPrice * f, 500)
    materials.push({ name, spec, qty, unit, unitPrice: price, total: qty * price })
  }

  // Floor covering with 8% waste allowance
  add('Pol qoplamasi', tier.floor, ceil(floorArea * 1.08 * 10) / 10, 'm²', tier.floorPrice)
  // Underlay
  add('Tagqoplama', 'Poliuretan 3 mm', ceil(floorArea * 1.05 * 10) / 10, 'm²', 9_000)
  // Skirting boards, 2.5 m planks
  add('Plintus', 'MDF 80 mm, 2.5 m', ceil((perimeter - DOOR.w) / 2.5), 'dona', 38_000)
  // Putty: 1.2 kg/m² on walls + ceiling, 25 kg bags
  add('Shpaklyovka', 'Knauf 25 kg', ceil(((wallArea + ceilingArea) * 1.2) / 25), 'qop', 68_000)
  // Primer: 0.15 L/m², 10 L cans
  add('Gruntovka', 'Chuqur kirib boruvchi, 10 L', ceil(((wallArea + ceilingArea) * 0.15) / 10), 'kanistr', 125_000)
  // Wall paint: 2 coats, 0.12 L/m² each, 10 L
  add('Devor bo‘yog‘i', 'Suv-dispersion, 10 L', ceil((wallArea * 0.24) / 10), 'bank', 290_000)
  // Ceiling paint
  add('Shift bo‘yog‘i', 'Oq matoviy, 10 L', ceil((ceilingArea * 0.24) / 10), 'bank', 260_000)
  // Serpyanka / tape for joints, 45 m rolls
  add('Serpyanka lenta', '45 m rulon', ceil(perimeter / 45) + 1, 'rulon', 14_000)
